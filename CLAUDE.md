...
| `template/scripts/knowledge.py` | LightRAG (>=1.4.13) knowledge graph builder (requires `OPENROUTER_API_KEY`). Supports optional OpenSearch backend (`LIGHTRAG_STORAGE=opensearch`). Builds from source extracts + PDFs, supports semantic queries, contradiction detection, evidence search. Venv auto-bootstraps on first run (`.venv/` at repo root). |
